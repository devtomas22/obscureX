...
import { loadTools, getTool, listTools as listToolsUtil } from './tools/toolLoader.js';
...
class ObscureXAgent {
  ...
  /**
   * Legacy compatibility - keep old tool structure
   */
  async _getOldStyleTools() {
    ...
    return this.tools.map(tool => ({
      ...
      execute: async (params) => {
        ...
        // For ML pipeline generation, pass anthropic client
        if (tool.name === 'generateMLPipeline') {
  ...
  // Remove old tool definitions - they're now in separate files
  // Keep the structure for backward compatibility
  async _initializeLegacyToolMethods() {
    await this._ensureToolsLoaded();
    
    // For backwards compatibility, bind tool methods
